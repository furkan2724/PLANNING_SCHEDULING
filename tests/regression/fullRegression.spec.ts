...
test.describe('Full Flow Regression Suite', () => {
  ...
  test('End-to-End Full Regression Flow @regression', async ({
    ...
    await test.step('🔐 Login to Planning Application', async () => {
      ...
      Logger.info('Logging into Planning application');

      await loginPage.navigateToPlanning();

      await loginPage.loginToPlanning();

      Logger.success('Planning Login successful');
    ...
    await test.step('➕ Create Demand', async () => {
      ...
      Logger.info(
        `Creating Demand for Product: ${productName}`
      );
      ...
      Logger.success(
        `Demand Created for Product: ${productName}`
      );
      ...
      Logger.success(
        `Demand updated for Product: ${productName}`
      );
    ...
    await test.step('✅ Verify Product in Planning', async () => {
      ...
      Logger.info(
        `Verifying Product in Planning: ${productName}`
      );

      await planningPage.verifyProductInPlanning(
        productName
      );

      Logger.success(
        `Product Verified in Planning: ${productName}`
      );
